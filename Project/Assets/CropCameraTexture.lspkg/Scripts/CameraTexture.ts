...
export class CameraTexture extends BaseScriptComponent {
  ...
  @ui.label(
    '<span style="color: #94A3B8; font-size: 11px;">Specify the UI image component and texture resources</span>',
  )
  @input
  ...
  @ui.label(
    '<span style="color: #94A3B8; font-size: 11px;">Define the crop region using normalized coordinates (-1 to 1) for left, right, bottom, and top boundaries</span>',
  )
  @input
  ...
  @ui.label(
    '<span style="color: #94A3B8; font-size: 11px;">Control logging output for this script instance</span>',
  )
  @input
  ...
  @input
  @hint(
    "Enable lifecycle logging (onAwake, onStart, onUpdate, onDestroy, etc.)",
  )
  ...
  private setupCamera(): void {
    try {
      ...
      ValidationUtils.assertNotNull(
        this.camModule,
        "Camera module not provided - please assign it in component settings",
      );
      ...
      if (this.enableLogging) {
        this.logger.debug(
          `Resolution: ${this.cameraRequest.imageSmallerDimension} (Editor: ${isEditor})`,
        );
      }
